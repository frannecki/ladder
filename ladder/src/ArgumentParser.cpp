    ...
    if(is_options[i]) {
      ...
      if(fields_options_.find(args[i]) != fields_options_.end()) {
        if(i+1 == args.size() || is_options[i+1]) {
          std::cerr << "[ArgumentParser] Option " << args[i] << " should be followed by a value" << std::endl;
          return false;
        }
        else {
          options_.insert(std::pair<std::string, std::string>(args[i], args[i+1]));
          ++i;
        }
      }
        ...
        std::cerr << "[ArgumentParser] No such option: " << args[i] << std::endl;
        return false;
      }
    }
  }
...
bool IArgumentParser::CheckRequired() { 
  for(auto iter = fields_options_.begin(); iter != fields_options_.end(); ++iter) {
    if(iter->second && options_.find(iter->first) == options_.end()) {
      std::cerr << "[ArgumentParser] String Option: " << iter->first << " is required." << std::endl;
      return false;
    }
  }
  
  for(auto iter = fields_enabled_.begin(); iter != fields_enabled_.end(); ++iter) {
    if(iter->second && enabled_.find(iter->first) == enabled_.end()) {
      std::cerr << "[ArgumentParser] Boolean Option: " << iter->first << " is required." << std::endl;
      return false;
    }
  }
