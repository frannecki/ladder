...
bool ProtobufCodec::ParseMessage(const std::string& packet, void*& message) const {
  ...
  std::string message_type_name = packet.substr(
    sizeof(message_type_name_length),
  ...
  uint32_t message_buffer_len = packet.size() - \
    kMinMessageLength - message_type_name_length;
  std::string message_buffer = packet.substr(
    sizeof(message_type_name_length) + message_type_name_length,
    message_buffer_len);
...
bool ProtobufCodec::HandleMessage(const ConnectionPtr& conn, void* message) const {
