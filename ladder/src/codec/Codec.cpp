...
const uint32_t Codec::kHeaderLength = \
...
void Codec::OnMessage(const ConnectionPtr& conn,
                      Buffer* buffer)
{
  std::string packet;
...
void Codec::Encapsulate(const void* message, std::string& packet) const {
  ...
  memcpy(header_str + sizeof(uint32_t),
         &checksum,
         sizeof(uint32_t));
...
void Codec::ParseAndHandleMessage(const ConnectionPtr& conn, const std::string& packet) const {
